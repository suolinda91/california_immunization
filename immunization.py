...
student_df = pd.read_csv('./data/StudentData.csv', parse_dates=True, encoding = "ISO-8859-1")


MMR_vaccinated_percent = student_df.groupby(['COUNTY', 'year']).apply(lambda x: x.nMMR.sum()/x.n.sum()*100).to_frame('pMMR').reset_index()
DTP_vaccinated_percent = student_df.groupby(['COUNTY', 'year']).apply(lambda x: x.nDTP.sum()/x.n.sum()*100).to_frame('pDTP').reset_index()
Polio_vaccinated_percent = student_df.groupby(['COUNTY', 'year']).apply(lambda x: x.nPolio.sum()/x.n.sum()*100).to_frame('pPolio').reset_index()
PersonalBeliefsExemption_percent = student_df.groupby(['COUNTY', 'year']).apply(lambda x: x.nPBE.sum()/x.n.sum()*100).to_frame('pPBE').reset_index()
PermanentMedicalExemption_percent = student_df.groupby(['COUNTY', 'year']).apply(lambda x: x.nPME.sum()/x.n.sum()*100).to_frame('pPME').reset_index()


PermanentMedicalExemption_perSchool = student_df.groupby('SCHOOL').apply(lambda x: x.nPME.sum()/x.n.sum()*100).to_frame('pPME').reset_index()
...
PermanentMedicalExemption_perSchool['pn'] = 100
PermanentMedicalExemption_perSchool.sort_values(by=['pPME'], ascending=False, inplace=True)
figure = plt.figure(figsize=(6, 15))
ax = figure.subplots()
sns.set_color_codes("pastel")
sns.barplot(x='pn', y='SCHOOL', data=PermanentMedicalExemption_perSchool.head(20), color='b', label='Total number of students')
sns.set_color_codes("muted")
sns.barplot(x='pPME', y='SCHOOL', data=PermanentMedicalExemption_perSchool.head(20),color='b', label='Number of Permanent Medical Exemptions')
...
### Wordcloud of schools with lowest immunization rate

SchoolList = MMR_perSchool['SCHOOL'].head(20).tolist()
SchoolList.extend(Polio_perSchool['SCHOOL'].head(20).tolist())
SchoolList.extend(DTP_perSchool['SCHOOL'].head(20).tolist())
map(str.lower, SchoolList)
stopwords = ['school', 'preschool', 'sch']
stopwords.extend(list(STOPWORDS))

wordcloud = WordCloud(stopwords=stopwords, background_color="white", width=550, height=550, margin=0, max_words=50).generate(" ".join(SchoolList))
